...
-----------------------
  ...
  M2. Driver Identification              → OLS Multiple Regression + diagnostics
...
------------------------------
  ...
  reported alongside practical significance.
...
# ── Paths ─────────────────────────────────────────────────────────────────────
DATA_PATH  = "/mnt/user-data/uploads/topik_survey_final.csv"
...
RENAME = {
    ...
    "\tquestion_diversity_rating"                    : "question_diversity_rating",
}
SCORE_COLS = [
...
def load(path: str) -> pd.DataFrame:
    """Load, rename, impute, and engineer features."""
...
def main():
    ...
    print("=" * 70)
    plot_fairness(df,          f"{OUTPUT_DIR}/05_fairness_consistency_plot.png")
    plot_opportunity_gap(proj, df, f"{OUTPUT_DIR}/06_opportunity_gap.png")
    plot_regression_drivers(reg,   f"{OUTPUT_DIR}/07_regression_drivers.png")
    print()
